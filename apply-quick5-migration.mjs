 ...
 * Usage: node apply-quick5-migration.mjs
 */
...
async function insertItems() {
  try {
    ...
    const itemsJson = JSON.parse(fs.readFileSync('items_with_real_images.json', 'utf8'));
    console.log(`📦 Loaded ${itemsJson.length} items from JSON\n`);

    // Prepare insert data
    const insertData = itemsJson.map(item => ({
      name: item.name,
