...
plan: "01"
subsystem: database-schema-and-infrastructure
tags: [schema, rls, tanstack-query, test-scaffolds, realtime]
dependency_graph:
  requires: [01-04]
  provides: [lists-schema, list_items-schema, item_history-schema, tanstack-query-provider, wave0-test-stubs]
  affects: [02-02, 02-03, 02-04]
tech_stack:
  added: ["@tanstack/svelte-query@6.1.0"]
  patterns: [QueryClientProvider-in-protected-layout, household-scoped-RLS-via-SECURITY-DEFINER-function]
key_files:
  created:
  ...
  modified:
    ...
    - package-lock.json
decisions:
  - "quantity is integer (not text) — text quantities deferred to Phase 3+"
  - "item_history excluded from supabase_realtime publication — write-only in Phase 2"
  - "QueryClient instantiated inside component body (not module level) to prevent SSR data leakage"
  - "Logout button removed from protected layout header — will live in Husstand tab (plan 02-02)"
metrics:
