...
plan: 01
subsystem: api
tags: [supabase, edge-functions, deno, postgres, playwright, gemini, kassal, open-food-facts]
requires:
  - phase: 03-store-layouts-and-category-ordering
    provides: household category taxonomy and the protected list-page Supabase client boundary
provides:
  - barcode_product_cache schema with TTL-backed found and not-found rows
  - authenticated barcode-lookup Edge Function with cache reuse, Kassal primary lookup, and Open Food Facts fallback
  - shared canonical barcode normalization helpers plus Gemini schema validation
  - Wave 0 barcode fixtures and Playwright scaffold for downstream scanner plans
affects: [04-02, 04-03, barcode-scanner-ui, test-fixtures]
tech-stack:
  added: [Supabase Edge Functions, Deno test coverage]
  patterns: [server-owned barcode lookup orchestration, canonical category enum validation, service-role-only cache access]
key-files:
  created:
  ...
  modified: []
key-decisions:
  - "barcode_product_cache stays server-only via RLS plus service_role grants so raw provider payloads never reach the browser"
  - "Gemini output is optional at runtime; invalid model output falls back to deterministic provider normalization instead of failing the lookup"
  - "12-digit UPC-A inputs normalize to a 13-digit cache key by prefixing 0 so one barcode resolves to one cached lookup path"
patterns-established:
  - "Barcode lookups terminate at one normalized DTO with a single found/not_found outcome regardless of provider fallback"
  - "Edge-function logic is dependency-injected so provider, cache, auth, and Gemini behavior can be covered with Deno tests"
requirements-completed: [BARC-02, BARC-03, BARC-04]
duration: 7min
