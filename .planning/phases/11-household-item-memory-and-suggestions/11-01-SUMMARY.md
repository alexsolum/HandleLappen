...
plan: 01
subsystem: database
tags: [supabase, postgres, suggestions, item-memory, playwright]
requires:
  - phase: 02-shopping-lists-and-core-loop
    provides: list items, household-scoped RLS, add-item mutation surface
  - phase: 03-store-layouts-and-category-ordering
    provides: category ids and picker fallback behavior
provides:
...
affects: [11-02, 11-03, list-add-flow]
tech-stack:
  added: []
  patterns: [trigger-backed remembered-item index, RPC-backed typeahead lookup]
key-files:
  created:
...
requirements-completed: [SUGG-01, SUGG-02, SUGG-03]
duration: 18min
