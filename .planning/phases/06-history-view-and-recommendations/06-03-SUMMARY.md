...
plan: 03
subsystem: ui
tags: [recommendations, navigation, mutations, playwright]
requires:
  - phase: 06-history-view-and-recommendations
    provides: history surface and recommendation queries
provides:
  - active Anbefalinger tab in BottomNav
  - active-list targeting context for recommendation routing
  - add-back actions with chooser fallback and quantity increment
affects: [navigation, history, recommendations, lists]
tech-stack:
  added: []
  patterns: [active-list route handoff, add-or-increment item mutation, toast-confirmed add-back]
key-files:
  created:
...
key-decisions:
  - "Only an obvious current list or a single household list can bypass the chooser."
  - "Add-back increments quantity by 1 instead of creating duplicate rows."
patterns-established:
  - "Bottom-nav recommendation routing carries active list context through the query string."
  - "History and recommendation rows share the same add-or-increment mutation path."
