...
plan: 01
subsystem: database
tags: [supabase, postgres, rls, playwright, onboarding]
requires:
  - phase: 02-shopping-lists-and-core-loop
    provides: shopping list tables, onboarding flow, Playwright helper patterns
provides:
  - categories, stores, and store_layouts schema for Phase 3
  - household category seed RPC wired into onboarding
  - Wave 0 category test scaffold and admin helpers
affects: [category-grouping, store-layouts, onboarding, testing]
tech-stack:
  added: []
  patterns: [household-scoped RLS, local supabase migration verification, generated database types]
key-files:
  created:
    - supabase/migrations/20260310000005_phase3_categories_stores.sql
    - tests/categories.spec.ts
    - tests/helpers/categories.ts
    - .planning/phases/03-store-layouts-and-category-ordering/deferred-items.md
  modified:
    - src/routes/velkommen/+page.server.ts
    - src/lib/types/database.ts
key-decisions:
  - "Seed failures in onboarding are logged and treated as non-fatal so household creation still succeeds."
  - "Supabase database types were regenerated from the local database rather than patched manually after adding the new RPC."
patterns-established:
  - "Phase 3 schema changes are verified against the local Supabase instance with `supabase db push --local` plus direct container checks."
  - "New database RPCs require a matching refresh of `src/lib/types/database.ts` before app code calls them."
